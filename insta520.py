    ...
    def start_tor(self):
        try:
            self.tor_process = stem.process.launch_tor_with_config(
                ...
                init_msg_handler=self.init_tor_progress,
                timeout=300,
                take_ownership=True
            )
            self.proxy = f"http://{self.tor_config['SocksPort']}"
            print("[\x1b[32m+\x1b[0m] Tor proxy started successfully")
        except Exception as err:
            print(f"[\x1b[31m!\x1b[0m] Error starting Tor: {err}")
            self.stop_tor()
    ...
    def stop_tor(self):
        if self.tor_process:
            try:
                ...
                print("[\x1b[32m+\x1b[0m] Tor proxy stopped successfully")
            except Exception as err:
                print(f"[\x1b[31m!\x1b[0m] Error stopping Tor: {err}")
        else:
            print("[\x1b[31m!\x1b[0m] Tor proxy is not running")

    def init_tor_progress(self, line):
